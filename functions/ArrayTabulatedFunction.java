   ...
   private int size;
    ...
    public double getFunctionValue(double x){
        ...
        for (int i = 0; i < size - 1; ++i){ // Перебараем в цикле все отрезки интервала
            ...
            if ((largeThan(x ,points[i].getCoorX()) || equal(x, points[i].getCoorX())) && largeThan(points[i+1].getCoorX(), x)){ // Если x входит в определённый отрезок
                return interpolate(x, points[i].getCoorX(), points[i].getCoorY(), points[i+1].getCoorX(), points[i+1].getCoorY());
            }
        }

        if (equal(x, points[size-1].getCoorX())){ // Если x является правой границей интервала
